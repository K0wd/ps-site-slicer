...
# -----------------------------------------------------------------------
# Parse test case IDs from plan (TC-01 … TC-NN, EC-01 … EC-NN)
# -----------------------------------------------------------------------

TC_IDS=$(grep -oE '^### (TC|EC)-[0-9]+' "$PLAN_FILE" | sed 's/### //' || true)

if [ -z "$TC_IDS" ]; then
    chomp_result "FAIL" "No test cases (TC-XX / EC-XX) found in plan"
    echo "ERROR: Could not extract test cases from $PLAN_FILE"
    exit 1
fi
...
extract_tc_section() {
    ...
    # Extract from "### TC-XX" heading to the next "###" heading or end of section
    awk "/^### ${tc_id}[^0-9]/,/^### (TC|EC)-[0-9]+[^0-9]|^## /" "$PLAN_FILE" \
        | head -60 \
        > "$out_file"
...
# Sort TC IDs in natural order (TC-01 … TC-NN, then EC-01 … EC-NN)
