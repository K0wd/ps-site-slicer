...
# Outputs a TICKETS: line with all keys for TestGenerator.sh to parse.
#
# Usage:
...
# Trim the raw result — strip bloated user objects, keep only what we need
STAGING_RESULT="/tmp/bite_step2_result.json"
echo "$RESULT" | python3 -c "
import json, sys
data = json.load(sys.stdin)
issues = data.get('issues', []) if isinstance(data, dict) else (data if isinstance(data, list) else [data])
trimmed = []
for issue in issues:
    f = issue.get('fields', {})
    trimmed.append({
...
json.dump({'issues': trimmed}, sys.stdout, indent=2)
" > "$STAGING_RESULT" 2>/dev/null || echo "$RESULT" > "$STAGING_RESULT"
...
echo "  Filter: $FILTER_LABEL — $TOTAL_FOUND ticket(s)"
echo ""
...
for issue in data.get('issues', []):
    if issue.get('key') == '$KEY':
        f = issue.get('fields', {})
        print(f\"{f.get('status','—')}|||{f.get('assignee','Unassigned')}|||{f.get('reporter','—')}\")
        break
" 2>/dev/null || echo "—|||—|||—")
...
# Output all tickets in a parseable line for TestGenerator.sh
