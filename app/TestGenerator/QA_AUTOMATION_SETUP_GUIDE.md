...
All files are in `app/TestGenerator/`:
...
Test Jira auth:
    ...
    python3 app/TestGenerator/jira_api.py test
...
Step 2: Make the runner executable:
    ...
    chmod +x app/TestGenerator/run-qa.sh
...
Step 4: Test manually on a single ticket:
    ...
    ./app/TestGenerator/run-qa.sh SM-XXXX
...
Step 5: Once the manual test works, install the launchd scheduler:
    ...
    chmod +x app/TestGenerator/setup-scheduler.sh
    ./app/TestGenerator/setup-scheduler.sh
...
## How It Works (what happens each run)
    ...
    6. Claude writes a test plan (app/TestGenerator/logs/{ticket}_plan.md)
...
Where to find what happened:
    ...
    app/TestGenerator/logs/qa_run_{timestamp}.log
        Runner log — did it start, find a ticket, finish

    app/TestGenerator/logs/claude_output_{ticket}_{timestamp}.txt
        Full Claude analysis output

    app/TestGenerator/logs/prompt_{timestamp}.txt
        Prompt that was sent to Claude

    app/TestGenerator/logs/{ticket}_plan.md
        Generated test plan for the ticket

    app/TestGenerator/logs/{ticket}_results.txt
